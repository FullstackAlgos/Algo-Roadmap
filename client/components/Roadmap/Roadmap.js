...
class Roadmap extends Component {
	...
	componentDidMount() {
  	...
  	componentDidUpdate() {
		...
		if(this.state.activeQ === "--" && this.props.userQuestions.length) {
			this.setState({activeQ: this.formatQualifyingQs(this.props.questions, this.props.userQuestions)[0].name})
		}
  	}
	...
	render() {
		...
		const orderedQs = this.formatQualifyingQs(this.props.questions, this.props.userQuestions);
